...
import { callMe } from '@/features/auth/api/auth.api'
...
export default function AdminPage() {
  ...
  const handleCallMe = async () => {
    try {
      console.log('🔥 Calling callMe() API...')
      const response = await callMe()
      console.log('✅ callMe() Response:', response)
    } catch (error) {
    ...
    <SidebarLayout>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-lg bg-white p-6 shadow">
          <h3 className="mb-4 text-lg font-medium text-gray-900">Debug API</h3>
          <Button onClick={handleCallMe} variant="secondary">
            Test callMe() API
          </Button>
          <p className="mt-2 text-xs text-gray-500">Check console for response</p>
        </div>
